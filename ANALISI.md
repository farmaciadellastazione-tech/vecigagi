...
- ✅ **Guardia anti-clobber anche in dialetti.html admin (stesso giorno)** — `adminGhPutFile` (usata da ☁️ Salva e da entrambe le PUT di 🚀 Promuovi) ora confronta il contenuto remoto con lo snapshot caricato (`adminDialFile`/`adminIndexFile` via `adminSnapshotPerPath` + `adminDecodificaContenutoGh`); se il file è cambiato l'operazione si ferma con spiegazione (fail-open se l'API omette il contenuto, file > ~1 MB). Nota: in una promozione, se il blocco scatta sulla seconda PUT (dialetti) dopo che la prima (index) è passata, i candidati promossi restano in staging — alla ricarica appaiono come = DUPLICATO e si ripromuovono senza danni. Test: +3 in `tests/importDialetti.test.mjs` (PUT eseguita/bloccata/fail-open, fetch mockato). Verifica dal vivo: salvataggio bloccato con avviso a remoto cambiato, eseguito a remoto identico.
...
### 🟡 Audio dialettale — fasi 1, 3 e 4 completate; fase 2 parziale

Il campo `audio:{sp:"file.mp3"}` con fallback TTS è in produzione dal 2026-07-19 (commit `91dca1b`), ma solo poche voci reali lo usano finora. Stato (vedi memoria `project_audio_dialettale`):
- **Fase 2**: propagare `entry.audio` ai call site rimanenti (suggerimenti, coniugazioni, recap) — parzialmente fatta, non riverificata dopo la fase 1.
- **Fase 3 (2026-07-22)**: fatta. `edit.html` (voce vocabolario, per lingua dialettale) ed `edit-storie.html` (frase/verso di una storia) hanno ora un pannello di registrazione: nome file proposto automaticamente (slug ASCII dalla pronuncia/dal titolo), il campo `audio` si aggiorna in memoria dopo l'upload, resta poi il normale "Salva su GitHub" per il testo.
- **Fase 4 (2026-07-22)**: fatta insieme alla 3, stesso lavoro. Registrazione in-browser via `MediaRecorder` (formato nativo webm/ogg opus, nessuna conversione mp3/dipendenza esterna), trim automatico del silenzio iniziale/finale + normalizzazione di picco opzionale via Web Audio API, upload binario diretto su GitHub (stesso PAT/meccanismo PUT già usato per il testo). **Limite noto**: nessuna riduzione del rumore di fondo (nessun equivalente `loudnorm`/`silenceremove` di ffmpeg) — il workflow manuale Drive→ffmpeg resta l'opzione per audio più puliti. Richiede HTTPS o `localhost` (non funziona da `file://`). Test: `tests/audioSlug.test.mjs`, `tests/base64Chunk.test.mjs`, `tests/trimSilenzio.test.mjs`.
- **Registrazione intera poesia (2026-07-22)**: in `edit-storie.html`, bottone "🎙️ Registra tutta la poesia…" a livello storia — un'unica ripresa con pause nette tra i versi, segmentazione automatica sulle pause (`trovaSegmentiAudio`, pura), pannello di revisione con anteprima per segmento (playback diretto dall'`AudioBuffer`, nessun encoding prima del salvataggio), slider "pausa minima" (300–1500 ms) e "unisci col precedente" (⤴) per il verso spezzato da una pausa interna. Salvataggio abilitato solo con N segmenti = M versi; file `audio/<lingua>/<slug>-NN.<ext>`, un commit per file, avanzamento visibile. Il pannello 🎙️ per-verso resta lo strumento di correzione fine (stesso nome file → sovrascrive). Test: `tests/segmentiAudio.test.mjs`.
